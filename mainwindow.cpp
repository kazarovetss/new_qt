...
void MainWindow::displayCoordinates(const QModelIndex &current, const QModelIndex &previous) { //отображение координат
    ...
    if (!tableModel) { //проверка на существование таблицы
       ...
       QString iconName = model->data(current, Qt::DisplayRole).toString(); //получение наименование и координат иконки

       QPointF position = iconCoordinates.value(iconName, QPointF(0, 0));

       tableModel->setItem(0, 1, new QStandardItem(QString::number(position.x()))); //установка координат в таблицу
...
void MainWindow::handleItemDropped(const QString &itemName, const QPointF &position) {
    ...
    iconCoordinates[itemName] = position; // Сохранение координат в QMap

    qDebug() << "Иконка с именем" << itemName << "была добавлена на позиции" << position; //вывод информации в консоль
}
